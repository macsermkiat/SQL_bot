...
def execute_both(
    ...
    Returns an ExecutionResult with rowcounts and errors from both runs.
    ...
    # --- generated SQL ---
    gen_row_count: int | None = None
    gen_error: str | None = None
    gen_exec_ms: float = 0.0
    try:
        result = db.execute_query(gen_sql, timeout_ms=timeout_ms, max_rows=max_rows)
        gen_row_count = result.row_count
        gen_exec_ms = result.execution_time_ms
        logger.debug("gen SQL OK: %d rows in %.0fms", gen_row_count, gen_exec_ms)
    except Exception as exc:
        gen_error = str(exc)
        logger.warning("gen SQL failed for %s: %s", ticket.ticket_number or ticket.id[:8], exc)

    # --- gold SQL (transpile first if not already PostgreSQL) ---
    gold_pg_sql, transpile_error = _transpile_to_postgres(ticket.gold_sql, ticket.gold_sql_dialect)
    if transpile_error:
        logger.warning("Transpile warning for %s: %s", ticket.id[:8], transpile_error)

    gold_row_count: int | None = None
    gold_error: str | None = None
    gold_exec_ms: float = 0.0
    try:
        result = db.execute_query(gold_pg_sql, timeout_ms=timeout_ms, max_rows=max_rows)
        gold_row_count = result.row_count
        gold_exec_ms = result.execution_time_ms
        logger.debug("gold SQL OK: %d rows in %.0fms", gold_row_count, gold_exec_ms)
    except Exception as exc:
        gold_error = str(exc)
        logger.warning("gold SQL failed for %s: %s", ticket.ticket_number or ticket.id[:8], exc)
